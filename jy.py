...
try:
    ...
    try:
        while True:
            ...
            print(round(np.amax(temperature), 2), round(np.amin(temperature), 2), min_temp, max_temp)
            print(round(np.amax(temp_norm), 2), round(np.amin(temp_norm), 2))
            temp_norm = np.clip(temp_norm, 0, 255)
            temp_norm = temp_norm.astype(np.uint8)

            threshold_value = 127
            _, temp_norm = cv2.threshold(temp_norm, threshold_value, 255, cv2.THRESH_BINARY)
            kernel = np.ones((5, 5), np.uint8)
            # temp_norm = cv2.erode(temp_norm, kernel, iterations=2)
            # temp_norm = cv2.dilate(temp_norm, kernel, iterations=2)

            # print(temp_norm)
            cv2.imshow('Lepton Radiometry', temp_norm)
            cv2.waitKey(1)
